...
def get_ews(M, mdot, a, cont_flnm):
    try:
        ...
        angle = np.arccos(np.linspace(1, -1, 40, endpoint = True)) #* (180.0/np.pi)
        return (angle, np.zeros(len(angle)))
...
for i in range(0, len(mass)):
    ...
    print(ews)
    plt.plot(np.cos(angle), ews, flags[i], label = r'$M/M_\odot = ' + r'{:g}'.format(mass[i]) + r'$')
...
ax.set_ylabel(r'$\mathrm{Fe\ K}\alpha\ \mathrm{EW}\ \left[\mathrm{eV}\right]$')
plt.tight_layout()
plt.savefig('ews_mass.pdf', bbox_inches = 'tight')
plt.show()
