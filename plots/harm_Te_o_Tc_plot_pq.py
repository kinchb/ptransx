...
T_C = 511.0 * np.array(harm_data_2['T_C'])

slice_data = T_e[:max_r_index,::,::]/T_C[:max_r_index,::,::]
...
plt.pcolormesh(theta, rad, np.log10(slice_data.T), cmap = cmap, vmin = 0.0, vmax = 5.0)
plt.colorbar(label = r'$\log \left( T_e/T_C \right)$', fraction = 0.046, pad = -0.15)
...
f = plt.gcf()
f.savefig('harm_Te_o_Tc.png', bbox_inches = 'tight', dpi = 600)
