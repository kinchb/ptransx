...
#       ax.set_xticks(np.pi/180.0 * np.array([0, 30, 60, 90, 120, 150, 180]))
    ...
    if which_plot == 'bsq_o_rho':
        ...
        im = ax.pcolormesh(theta, rad, np.log10(slice_data.T), cmap = cmap, vmin = -4.0, vmax = 4.0, shading = 'gourand')
    else:
        cmap = plt.get_cmap('jet')
        im = ax.pcolormesh(theta, rad, np.log10(slice_data.T), cmap = cmap, vmin = -1.0, vmax = 5.0, shading = 'gourand')
...
if which_plot == 'bsq_o_rho':
    ...
    fig.colorbar(im, ax = axs, label = r'$\log \left( B^2/\rho \right)$', shrink = 0.6, pad = -0.05, ticks = ticks)
    plt.savefig('harm_bsq_o_rho_grid.png', dpi = 600, bbox_inches = 'tight')
else:
    ticks = [-1, 0, 1, 2, 3, 4, 5]
    fig.colorbar(im, ax = axs, label = r'$\log \left( B^2/u \right)$', shrink = 0.6, pad = -0.05, ticks = ticks)
    plt.savefig('harm_bsq_o_u_grid.png', dpi = 600, bbox_inches = 'tight')
