...
dir = '../harm_data/oldcool_data'

# harm_data_1 = h5py.File(dir + '/KDHARM0.000500.h5', 'r')
harm_data_2 = h5py.File(dir + '/KDHARM0.RADFLUX.011000.h5', 'r')

cool = np.array(harm_data_2['coolfunc_corona']) # + np.array(harm_data_2['coolfunc_disk'])
...
plt.pcolormesh(theta, rad, ((1.0/slice_data).T), cmap = cmap, vmin = 0.0, vmax = 2.0)
plt.colorbar(label = r'$\log \left( \mathrm{TT}_{1000M} / \mathrm{TT}_{-1M} \right)$', fraction = 0.046, pad = -0.15)
...
f = plt.gcf()
f.savefig('harm_tt_o_tt.png', bbox_inches = 'tight', dpi = 600)
