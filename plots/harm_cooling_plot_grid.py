...
#   im = ax.pcolormesh(theta, rad, np.log10(slice_data.T), cmap = cmap, vmin = -10.0, vmax = -5.0, shading = 'gourand')
    im = ax.pcolormesh(theta, rad, np.log10(slice_data.T), cmap = cmap, vmin = 9.0, vmax = 16.0, shading = 'gourand')
...
fig.colorbar(im, ax = axs, label = r'$\log \mathcal{L} \left[\mathrm{erg}\ \mathrm{s}^{-1}\ \mathrm{cm}^{-3} \right]$', shrink = 0.6, pad = -0.05, ticks = ticks)

plt.savefig('harm_cool_grid.png', dpi = 600, bbox_inches = 'tight')
