...
urad = np.ma.masked_where(urad == 0.0, urad)

slice_data = np.mean(urad[:max_r_index,::,::], axis = 2)
...
urad_conv = (4 * np.pi * c**4 * mdot)/(kappa * G * m_bh * 3.0e-4 * eta)

slice_data *= urad_conv

plt.pcolormesh(theta, rad, np.log10(slice_data.T), cmap = cmap, vmin = 10.0, vmax = 12.0)
...
f = plt.gcf()
f.savefig('harm_urad.png', bbox_inches = 'tight', dpi = 600)
