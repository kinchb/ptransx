...
#   return (e_grid, ratio, ew)
    ...
    return (e_grid, line_spec[ndx] * 2.417989e14 * (1.0/(2.0 * np.pi)) * (2.0/len(line_spec)), ew)
...
for i in range(0, len(abund)):
    ...
    plt.plot(nu/1.0e3, nu * ratio, flags[i], label = r'$[\mathrm{Fe}]/[\mathrm{Fe}]_\odot = ' + r'{:g}'.format(abund[i]) + r'$')
...
plt.savefig('line_flux_abund.pdf', bbox_inches = 'tight')
