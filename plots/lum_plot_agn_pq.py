...
dir = [root + 'h3d_3_a0_01/', root + 'h3d_1e6_a0_01/']
...
for i in range(0, len(mdot)):
    for j in range(0, len(spin)):
        flnm[i][j] = dir[i][j] + 'scat_spec.1000.1.h5'
"""

for i in range(0, len(mass)):
    flnm[i] = dir[i] + 'scat_spec.1000.1.h5'

# fig, axs = plt.subplots(len(mdot), len(spin), sharex = True, sharey = True, figsize = (9, 6))

fig, axs = plt.subplots(nrows = len(mass), ncols = 1, sharex = True, sharey = False, figsize = (6, 9))
...
"""
for i in range(0, len(mass)):
    nu, Lnu = get_spec(mass[i], 0.01, 0.0, dir[i], flnm[i])
    make_plot(axs[i], nu, Lnu, mass[i], 0.01, 0.0)
...
# fig.savefig('spec_mass_grid.pdf', bbox_inches = 'tight')

plt.show()

flags = [r'r-', r'k-']

mass_label = [r'10', r'10^6']

for i in range(0, len(mass)):
    nu, Lnu = get_spec(mass[i], 0.01, 0.0, dir[i], flnm[i])
    plt.plot(nu/1.0e3, nu * Lnu, flags[i], label = r'$M/M_\odot = ' + mass_label[i] + r'$')
plt.legend(loc = 'upper left', frameon = False)
ax = plt.gca()
ax.set_xscale('log')
ax.set_yscale('log')
ax.xaxis.set_minor_formatter(matplotlib.ticker.NullFormatter())
ax.set_xticks((1.0e-3, 1.0e-2, 1.0e-1, 1.0e0, 1.0e1, 1.0e2, 1.0e3))
ax.set_xticklabels((r'$10^{-3}$', r'$10^{-2}$', r'$10^{-1}$', r'$10^0$', r'$10^1$', r'$10^2$', r'$10^3$'))
ax.set_xlim([0.001, 1.0e3])
ax.set_ylim([1.0e-4, 1.0e-2])
ax.set_xlabel(r'$\mathrm{energy}\ \left[\mathrm{keV}\right]$')
ax.set_ylabel(r'$\varepsilon L_\varepsilon / L_\mathrm{Edd}$')
plt.tight_layout()
plt.savefig('agn_to_xrb_comp.pdf', bbox_inches = 'tight')
plt.show()