...
def get_spec(M, mdot, a, spec_dir, spec_flnm):
    ...
    try:
        ...
        return (e_grid, np.ones(len(e_grid)))
    cont_spec = np.array(infile['/data']).T
    infile.close()
    line_flnm = spec_flnm.replace('_c_', '_l_')
    try:
...
for i in range(0, len(mass)):
    nu, Lnu = get_spec(mass[i], 0.01, 0.0, dir[i], flnm[i])
    plt.plot(nu/1.0e3, nu * Lnu, flags[i], label = r'$M/M_\odot = ' + r'{:g}'.format(mass[i]) + r'$')
...
ax.set_ylabel(r'$\varepsilon L_\varepsilon / L_\mathrm{Edd}$')
plt.tight_layout()
plt.savefig('spec_mass_one_plot.pdf', bbox_inches = 'tight')
plt.show()