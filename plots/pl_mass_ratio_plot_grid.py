...
fig, axs = plt.subplots(len(mass), 1, sharex = True, sharey = True, figsize = (6, 9))

for i in range(0, len(mass)):
    for k in range(0, len(angle_list)):
        nu, ratio, ew = get_line_by_pl(mass[i], 0.01, 0.0, dir[i], flnm[i], spec_ndx_list[k])
        make_plot(axs[i], nu, ratio, 0.01, 0.0, flags[k], k)
